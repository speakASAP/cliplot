...
export function liveCheckoutPreflight() {
  ...
  return {
    ...
    validation: {
      ...
      // Cliplot reads no payment status: it holds no payments:read scope and
      // Payments is the sole owner of payment state.
      paymentStatus: 'not_read',
    },
    missing,
    next: fullyReady
...
function paymentReadScopeRateLimitedResult(evidence = {}, now = Date.now()) {
  ...
  return {
    ...
    generatedAt: new Date().toISOString(),
    service: serviceConfig.serviceName,
    // Generic 429 shape, reused by the Warehouse/Orders/Notifications guarded
    // validators. Cliplot requests no payments:read scope and reads no payment
    // status, so no endpoint or scope is asserted here.
    keyPresent: true,
...
export async function checkoutLiveReadinessHandoffEvidencePacket() {
  ...
  return {
    ...
    paymentReadScopePolicy: {
      // Cliplot does not request payments:read and reads no payment status.
      // Payments is the sole owner of payment state.
      required: false,
...
export async function ownerBoundedWindowReadinessHandoffPacket() {
  ...
  // Cliplot does not request payments:read and reads no payment status, so
  // there is no read-scope or reconciliation evidence to hand off.
  const ownerPaymentReadScopeStatus = 'not_requested';
  const ownerPaymentReadScopeFreshness = null;
  const ownerExternalStatusReconciliation = null;
  const ownerReconciledPaymentStatus = null;
...
export async function liveCheckoutApprovalPacket() {
  ...
  return {
    ...
    paymentBoundary: {
      // Cliplot holds no payments:read scope and reads no payment status.
      // Payments remains the sole owner; only the create/callback boundary is
      // reported here.
      statusRead: false,
      callbackReplayPolicy: callbackPolicy.status,
      paymentCreateApproval: paymentCreateApproval.status,
      ...
      callbackPersistenceEnabled: callbackPolicy.callbackPersistence,
...
export async function revenueClosurePacket() {
  ...
  return {
    ...
    payment: {
      // Cliplot does not read payment status: it holds no payments:read scope
      // and keeps no status of its own. Only create-validation metadata is
      // reported here; Payments remains the sole owner of payment state.
      paymentCreateApproval: paymentCreateApproval.status,
      paymentCreateValidation: paymentCreateApproval.validation?.status || null,
      readyForOwnerPaymentApproval: paymentCreateApproval.approvalIdMayBeRecordedAfterOwnerAcceptance,
      statusRead: false,
