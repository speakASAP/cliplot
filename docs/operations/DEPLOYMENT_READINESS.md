...
- The callback replay policy gate may return `approved_callback_replay_policy_metadata_execution_disabled` once policy metadata is approved, and the callback persistence approval packet must return `approval_required_callback_persistence_storage_backend`; both must keep `callbackPersistence=false`, `callbackReplayEnabled=false`, `mutation=false`, `persistence=false`, and `providerCall=false` until callback storage and replay execution rollout are approved.
