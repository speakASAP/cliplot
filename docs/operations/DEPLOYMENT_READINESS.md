...
- The callback replay policy gate may return `approved_callback_replay_policy_metadata_execution_disabled` once policy metadata is approved, and the callback persistence approval packet must return `approval_required_callback_persistence_storage_backend`; both must keep `callbackPersistence=false`, `callbackReplayEnabled=false`, `mutation=false`, `persistence=false`, and `providerCall=false` until callback storage and replay execution rollout are approved. The callback persistence packet may include `storageBackendProposal`, `rolloutPlan`, and `replayDryRunContract` metadata, and the callback replay rollout packet may include bounded execution-window and synthetic dry-run metadata; those sections are proposal-only and must keep runtime enablement, callback persistence, replay execution, live status writes, provider calls, notification sends, and storage writes false.
- The live status write approval packet must return `approval_required_live_status_write` and keep `liveStatusWritesEnabled=false`, `liveStatusWritesNow=false`, `callbackPersistence=false`, `callbackReplayEnabled=false`, `mutation=false`, `persistence=false`, and `providerCall=false` until separate owner approval, callback persistence storage, replay rollout, bounded write window, validation owner, and rollback owner evidence exist.
- The callback persistence storage contract/checklist packet must return `proposal_metadata_recorded_approval_required` and keep `callbackPersistence=false`, `callbackReplayEnabled=false`, `liveStatusWritesNow=false`, `mutation=false`, `persistence=false`, and `providerCall=false` until storage, rollout, retention/deletion, uniqueness/conflict, replay, live-status-write, rollback, and validation approvals exist.
