...
- Payment-create approval evidence must use only `POST /payments/validate-create`, return `ready_for_owner_payment_create_approval_metadata`, and keep `ENABLE_LIVE_PAYMENT_CREATE=false`, `CLIPLOT_LIVE_PAYMENT_APPROVAL_ID=""`, `mutation=false`, `persistence=false`, and `providerCall=false` until owner acceptance and a separate bounded live payment execution window.
- Notification-send approval evidence must use only `POST /notifications/validate`, return `ready_for_owner_notification_send_approval_metadata`, and keep `ENABLE_LIVE_NOTIFICATIONS=false`, `CLIPLOT_LIVE_NOTIFICATION_APPROVAL_ID=""`, `mutation=false`, `persistence=false`, `providerCall=false`, and `notificationSent=false` until owner acceptance and a separate bounded live notification execution window.
- Bounded payment-create execution readiness must use only `GET /api/payments/create-execution-window-packet` and the blocked `POST /api/payments/create-bounded-executor` stub during validation. It must return `approval_required`, `liveExecutionAllowed=false`, `paymentCreated=false`, `mutation=false`, `persistence=false`, and `providerCall=false` by default, and the script must also pass after metadata approval when the live flag or concrete window still blocks execution; `/payments/create` remains forbidden until owner approval, concrete window, idempotency, duplicate check, rollback owner, and validation owner are all present.
- Bounded notification-send execution readiness must use only `GET /api/notifications/send-execution-window-packet` and the blocked `POST /api/notifications/send-bounded-executor` stub during validation. It must return `approval_required`, `liveExecutionAllowed=false`, `notificationSent=false`, `mutation=false`, `persistence=false`, and `providerCall=false` by default, and the script must also pass after metadata approval when the live flag or concrete window still blocks execution; `/notifications/send` remains forbidden until owner approval, concrete window, idempotency, duplicate check, rollback owner, and validation owner are all present.
