...
- `/api/payments/status` now uses only the Payments DB-only by-order-id snapshot
  ...
  notifications.
- Payment status storage ownership is approved for the shared Payments source-of-truth passive read path through `CLIPLOT_PAYMENT_STORAGE_OWNERSHIP_APPROVAL_ID`, while Cliplot-local storage writes remain disabled. `GET /api/payments/status-storage-readiness` still returns `blocked_storage_backend_not_approved` for callback/local storage writes; `GET /api/payments/status-persistence-decision` still returns `decision_recorded_approval_required` because live writes remain blocked; `GET /api/payments/status-mapping-ownership` returns `approved_order_payment_status_mapping_ownership` when ADR-006 owner metadata is configured. ADR-006 keeps Orders authoritative for order lifecycle, Payments authoritative for payment status, and Cliplot non-authoritative.
