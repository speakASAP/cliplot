...
- The callback replay execution rollout packet is wired as `GET /api/payments/callback-replay-execution-rollout-proposal-packet` and `npm run readiness:payment-callback-replay-rollout`. It can report `approved_callback_replay_execution_metadata_execution_disabled` when replay execution metadata, bounded window, rollback owner, and validation owner are recorded, while preserving `callbackReplayEnabled=false`, `callbackPersistence=false`, `replayExecutionAllowed=false`, `mutation=false`, `persistence=false`, `providerCall=false`, storage writes disabled, live status writes disabled, provider calls disabled, and notification sends disabled.
- The live status write approval packet is wired as `GET /api/payments/live-status-write-approval-packet` and `npm run readiness:payment-live-status-write`. It can report `approved_live_status_write_metadata_execution_disabled` when the approval metadata, bounded window, rollback owner, and validation owner are recorded, while preserving `liveStatusWritesEnabled=false`, `liveStatusWritesNow=false`, `callbackReplayEnabled=false`, `callbackPersistence=false`, `mutation=false`, `persistence=false`, `providerCall=false`, storage writes disabled, provider-backed reads disabled, payment creation disabled, and notification sends disabled.
- The callback persistence storage contract/checklist packet is wired as `GET /api/payments/callback-persistence-storage-contract-packet`, `GET /api/payments/callback-persistence-storage-approval-checklist-packet`, `npm run readiness:payment-callback-storage-contract`, and `npm run readiness:payment-callback-storage-approval-checklist`. It records metadata-only identity, idempotency, uniqueness, retention/deletion, rollback, validation, and approval-flag requirements for the future Payments-owned callback event projection while preserving `callbackPersistence=false`, `callbackReplayEnabled=false`, `liveStatusWritesNow=false`, `mutation=false`, `persistence=false`, and `providerCall=false`.
- The callback persistence approval packet now includes metadata-only `storageBackendProposal`, `rolloutPlan`, and `replayDryRunContract` sections for the future Payments-owned callback event projection. A dedicated `GET /api/payments/callback-storage-backend-proposal-packet` and `npm run readiness:payment-callback-storage-proposal` expose the same proposal for owner review. These sections are proposal-only and preserve `callbackPersistence=false`, `callbackReplayEnabled=false`, `mutation=false`, `persistence=false`, `providerCall=false`, storage writes disabled, and live status writes disabled.
...
- `/api/payments/status` now uses only the Payments DB-only by-order-id snapshot
  ...
  notifications.
- Payment status storage ownership is approved for the shared Payments source-of-truth passive read path through `CLIPLOT_PAYMENT_STORAGE_OWNERSHIP_APPROVAL_ID`, while Cliplot-local storage writes remain disabled. `GET /api/payments/status-storage-readiness` may return `approved_payment_status_storage_metadata_execution_disabled` when callback storage, rollout, retention, uniqueness, replay, and live-status-write metadata approvals are recorded; it still keeps `storage.configured=false`, `callbackPersistence=false`, `currentStatusPersistence=false`, `mutation=false`, `persistence=false`, and `providerCall=false`. `GET /api/payments/status-persistence-decision` may return `approved_payment_status_persistence_decision_metadata_execution_disabled` when the same metadata blockers are closed; live writes remain separately blocked. `GET /api/payments/status-mapping-ownership` returns `approved_order_payment_status_mapping_ownership` when ADR-006 owner metadata is configured. ADR-006 keeps Orders authoritative for order lifecycle, Payments authoritative for payment status, and Cliplot non-authoritative.
