...
- The callback replay execution rollout packet is wired as `GET /api/payments/callback-replay-execution-rollout-proposal-packet` and `npm run readiness:payment-callback-replay-rollout`. It records bounded execution-window and synthetic dry-run metadata while preserving `callbackReplayEnabled=false`, `callbackPersistence=false`, `replayExecutionAllowed=false`, `mutation=false`, `persistence=false`, `providerCall=false`, storage writes disabled, live status writes disabled, provider calls disabled, and notification sends disabled.
- The live status write approval packet is wired as `GET /api/payments/live-status-write-approval-packet` and `npm run readiness:payment-live-status-write`. It records a metadata-only bounded write-window proposal and dry-run plan while preserving `liveStatusWritesEnabled=false`, `liveStatusWritesNow=false`, `callbackReplayEnabled=false`, `callbackPersistence=false`, `mutation=false`, `persistence=false`, `providerCall=false`, storage writes disabled, provider-backed reads disabled, payment creation disabled, and notification sends disabled.
