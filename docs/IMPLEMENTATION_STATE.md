...
- The guarded payment status storage readiness endpoint is wired as `GET /api/payments/status-readiness` and `npm run readiness:payment-status`. It documents the future Payments read contract `GET /payments/{paymentId}` with `payments:read` scope and flags the provider-refresh risk for pending Stripe/card reads, but keeps Cliplot blocked at `blocked_pending_provider_backed_status_contract` until there is approved persisted `paymentId` storage, external-order/payment mapping, and owner approval for provider-backed status reads. Current status remains `payment_status_guarded_no_persistence` with `mutation=false`, `persistence=false`, and `providerCall=false`. Guarded callback/status readiness now also exposes a Czech customer-safe payment status mapping for `pending`, `processing`, `completed`, `failed`, `cancelled`, and `refunded` without persistence or provider calls.
