...
if [ "$critical_failed" -eq 0 ]; then
  ...
  run_step payment_callback_storage_contract npm run readiness:payment-callback-storage-contract -- "$BASE_URL"
  run_step payment_callback_storage_approval_checklist npm run readiness:payment-callback-storage-approval-checklist -- "$BASE_URL"
  ...
  run_step live_checkout_execution_evidence npm run readiness:live-checkout-execution-evidence -- "$BASE_URL"
  run_step live_checkout_execution_request npm run readiness:live-checkout-execution-request -- "$BASE_URL"
  run_step auth_wallet_runtime_checkout_evidence npm run readiness:auth-wallet-runtime-checkout-evidence -- "$BASE_URL"
  run_step live_flags_operator_preflight npm run readiness:live-flags-operator-preflight -- "$BASE_URL"
  ...
  run_step payment_callback_to_status_write_dry_run_contract npm run readiness:payment-callback-to-status-write-dry-run-contract -- "$BASE_URL"
  run_step payment_status_write_owner_review npm run readiness:payment-status-write-owner-review -- "$BASE_URL"
  run_step payment_external_status_reconciliation_preflight npm run readiness:payment-external-status-reconciliation-preflight -- "$BASE_URL"
  ...
  run_step live_smoke_execution_checklist npm run readiness:live-smoke-execution-checklist -- "$BASE_URL"
  run_step live_smoke_contract npm run readiness:live-smoke-contract -- "$BASE_URL"
